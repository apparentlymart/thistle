...
function Compile(opts) {
    ...
    function collectDirectives(node) {
        ...
        var nodeDirectives = [];
        var exclusiveDirective;
        for (var i = 0; i < directives.length; i++) {
            var maybeDirective = directives[i];
            if (maybeDirective.match(node)) {
                if (maybeDirective.exclusive) {
                    if (exclusiveDirective) {
                        throw new CompileError(
                            'Cannot use ' + exclusiveDirective.selector + ' and ' +
                            maybeDirective.selector + ' on the same element.'
                        );
                    }
                    else {
                        exclusiveDirective = maybeDirective;
                    }
                }
                nodeDirectives.push(maybeDirective);
            }
        }
        nodeDirectives.sort(
            function (a, b) {
