...
function Compile(opts) {
    ...
    function makeElementLinkFn(element) {
        ...
        if (elementDirectives.template) {
            ...
            return elementDirectives.template.compile(element);
        }
        ...
        if (elementDirectives.component) {
            ...
            componentLinkFn = elementDirectives.component.compile(element);
        }

        var decoratorLinkFns = elementDirectives.decorators.map(
            function (decoratorDirective) {
                return decoratorDirective.compile(element);
            }
        );
        ...
        if (componentLinkFn) {
            ...
            return function linkComponentElement(scope, parentNode) {
                var children = [];
                componentRoot.children = children;
                componentLinkFn(scope, componentRoot);
    ...
    return function compile(template, opts) {
        ...
        ret.appendTo = function linkAndAppend(scope, node) {
            if (linkFn) {
                linkFn(scope, node);
            }
        };
