...
function combineLinkFuncs(funcs) {
    ...
    // Simple path: if we've only been given one function then
    ...
    var calls = flattenedFuncs.map(
        function (func, index) {
            return '    func' + index + '(scope, nodes);' + (func.name ? ' // ' + func.name : '') + '\n';
        }
    );
    ...
    var builderSrc = 'return function combinedLink(scope, nodes) {\n' + calls.join('') + '};\n';
