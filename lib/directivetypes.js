...
function DirectiveTypes(opts) {
    ...
    function DecoratorDirective(def) {
        ...
        this.match = compileSelector(selector);
        this.type = 'decorator';
        this.compile = function compileDecoratorDirective(tElement, context) {
            var directiveLinkFns = directiveCompile(tElement.name, tElement.attribs, context);

            if (typeof directiveLinkFns === 'function') {
                directiveLinkFns = {
                    post: directiveLinkFns
                };
            }

            if (! directiveLinkFns.pre && ! directiveLinkFns.post) {
                throw new Error('Decorator directive must return at least one of pre and post functions.');
            }

            var directivePreLink = directiveLinkFns.pre;
            var directivePostLink = directiveLinkFns.post;

            return {
                pre: function (scope) {
                    if (directivePreLink) {
                        directivePreLink(scope);
                    }
                },
                post: function (scope, iElement) {
                    if (directivePostLink) {
                        directivePostLink(scope, iElement.attribs);
                    }
                }
            }
        };
